...
class GerarPdfFretesJob implements ShouldQueue
{
    use Dispatchable;
    ...
    {
        $this->onQueue('fretes-exportacao');
    }

    public function handle(FreteQuery $freteQuery): void
    {
        $exportacao = FreteExportacao::query()->find($this->exportacaoId);
        ...
        $exportacao->forceFill([
            'status' => FreteExportacao::STATUS_PROCESSANDO,
            'started_at' => $exportacao->started_at ?? now(),
            'erro_mensagem' => null,
        ])->save();

        try {
            $filtros = $freteQuery->normalizarFiltros($exportacao->filtros ?? []);
            $query = $freteQuery->aplicarFiltros(
                Frete::query()->with('veiculo:id,id_sbs,nome'),
                $filtros,
            );
            ...
            $geradoEm = now();
            $arquivoNome = 'fretes_'.$geradoEm->format('Ymd_His').'_'.$exportacao->uuid.'.pdf';
            $arquivoPath = 'fretes/exportacoes/'.$arquivoNome;
            $geradoPor = $exportacao->user_id !== null
                ? User::query()->whereKey($exportacao->user_id)->value('name')
                : null;

            $pdf = Pdf::loadView('admin.fretes.pdf', [
                'fretes' => $fretes,
                'filtros' => $filtros,
                'geradoEm' => $geradoEm,
                'geradoPor' => $geradoPor ?: '—',
            ...
            $exportacao->forceFill([
                'status' => FreteExportacao::STATUS_CONCLUIDO,
    ...
    private function marcarFalha(FreteExportacao $exportacao, Throwable $e): void
    {
        Log::warning('Exportação PDF de Fretes falhou', [
        ...
        $exportacao->forceFill([
            'status' => FreteExportacao::STATUS_FALHOU,
            'finished_at' => now(),
