...
namespace App\Jobs\Fornecedores;
...
class GerarPdfFornecedoresJob implements ShouldQueue
{
    use Dispatchable;
    ...
    {
        $this->onQueue('fornecedores-exportacao');
    }

    public function handle(FornecedorQuery $fornecedorQuery): void
    {
        $exportacao = FornecedorExportacao::query()->find($this->exportacaoId);
        ...
        $exportacao->forceFill([
            'status' => FornecedorExportacao::STATUS_PROCESSANDO,
            'started_at' => $exportacao->started_at ?? now(),
            'erro_mensagem' => null,
        ])->save();

        try {
            $filtros = $fornecedorQuery->normalizarFiltros($exportacao->filtros ?? []);
            $query = $fornecedorQuery->aplicarFiltros(
                Fornecedor::query()->select([
                    'fornecedores.id_cigam',
                    'fornecedores.razao_social',
                    'fornecedores.fantasia',
                    'fornecedores.cnpj_cpf',
                    'fornecedores.id_estado',
                ]),
                $filtros,
            );
            ...
            $geradoEm = now();
            $arquivoNome = 'fornecedores_'.$geradoEm->format('Ymd_His').'_'.$exportacao->uuid.'.pdf';
            $arquivoPath = 'fornecedores/exportacoes/'.$arquivoNome;
            $geradoPor = $exportacao->user_id !== null
                ? User::query()->whereKey($exportacao->user_id)->value('name')
                : null;

            $pdf = Pdf::loadView('admin.fornecedores.pdf', [
            ...
            $exportacao->forceFill([
                'status' => FornecedorExportacao::STATUS_CONCLUIDO,
    ...
    private function marcarFalha(FornecedorExportacao $exportacao, Throwable $e): void
    {
        Log::warning('Exportação PDF de Fornecedores falhou', [
        ...
        $exportacao->forceFill([
            'status' => FornecedorExportacao::STATUS_FALHOU,
            'finished_at' => now(),
