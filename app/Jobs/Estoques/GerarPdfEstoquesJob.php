...
class GerarPdfEstoquesJob implements ShouldQueue
{
    use Dispatchable;
    ...
    {
        $this->onQueue('estoques-exportacao');
    }

    public function handle(EstoqueQuery $estoqueQuery): void
    {
        $exportacao = EstoqueExportacao::query()->find($this->exportacaoId);
        ...
        $exportacao->forceFill([
            'status' => EstoqueExportacao::STATUS_PROCESSANDO,
            'started_at' => $exportacao->started_at ?? now(),
            'erro_mensagem' => null,
        ])->save();

        try {
            $filtros = $estoqueQuery->normalizarFiltros($exportacao->filtros ?? []);
            $query = $estoqueQuery->aplicarFiltros(
                Estoque::query()
                    ->select('estoques.*')
                    ...
                    ->with([
                        ...
                        'fruta:id,nome,id_cigam,unidade_medicao',
                    ]),
                $filtros,
            );
            ...
            $geradoEm = now();
            $arquivoNome = 'estoques_'.$geradoEm->format('Ymd_His').'_'.$exportacao->uuid.'.pdf';
            $arquivoPath = 'estoques/exportacoes/'.$arquivoNome;
            $geradoPor = $exportacao->user_id !== null
                ? User::query()->whereKey($exportacao->user_id)->value('name')
                : null;

            $pdf = Pdf::loadView('admin.estoques.pdf', [
                'estoques' => $estoques,
                'filtros' => $filtros,
                'geradoEm' => $geradoEm,
                'geradoPor' => $geradoPor ?: '—',
            ...
            $exportacao->forceFill([
                'status' => EstoqueExportacao::STATUS_CONCLUIDO,
    ...
    private function marcarFalha(EstoqueExportacao $exportacao, Throwable $e): void
    {
        Log::warning('Exportação PDF de Estoques falhou', [
        ...
        $exportacao->forceFill([
            'status' => EstoqueExportacao::STATUS_FALHOU,
            'finished_at' => now(),
