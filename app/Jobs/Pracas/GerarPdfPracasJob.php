...
class GerarPdfPracasJob implements ShouldQueue
{
    use Dispatchable;
    ...
    {
        $this->onQueue('pracas-exportacao');
    }

    public function handle(PracaQuery $pracaQuery): void
    {
        $exportacao = PracaExportacao::query()->find($this->exportacaoId);
        ...
        $exportacao->forceFill([
            'status' => PracaExportacao::STATUS_PROCESSANDO,
            'started_at' => $exportacao->started_at ?? now(),
            'erro_mensagem' => null,
        ])->save();

        try {
            $filtros = $pracaQuery->normalizarFiltros($exportacao->filtros ?? []);
            $query = $pracaQuery->aplicarFiltros(
                Praca::query()
                    ->select(['id', 'nome', 'id_unidade_negocio'])
                    ->with('unidadeNegocio:id,nome,id_cigam'),
                $filtros,
            );
            ...
            $geradoEm = now();
            $arquivoNome = 'pracas_'.$geradoEm->format('Ymd_His').'_'.$exportacao->uuid.'.pdf';
            $arquivoPath = 'pracas/exportacoes/'.$arquivoNome;
            $geradoPor = $exportacao->user_id !== null
                ? User::query()->whereKey($exportacao->user_id)->value('name')
                : null;

            $pdf = Pdf::loadView('admin.pracas.pdf', [
                'pracas' => $pracas,
                'filtros' => $filtros,
                'geradoEm' => $geradoEm,
                'geradoPor' => $geradoPor ?: '—',
            ...
            $exportacao->forceFill([
                'status' => PracaExportacao::STATUS_CONCLUIDO,
    ...
    private function marcarFalha(PracaExportacao $exportacao, Throwable $e): void
    {
        Log::warning('Exportação PDF de Praças falhou', [
        ...
        $exportacao->forceFill([
            'status' => PracaExportacao::STATUS_FALHOU,
            'finished_at' => now(),
