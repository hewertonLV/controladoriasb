...
class FrutaQuery
{
    ...
    private const ALLOWED_SORTS = [
        ...
        'kg_por_unidade_medicao' => 'kg_por_unidade_medicao',
        'created_at' => 'created_at',
    ];

    private const NUMERIC_SORTS = [
        'id_cigam' => true,
        'kg_por_unidade_medicao' => true,
    ];
        ...
        if ($filtros['search'] !== '') {
            ...
            $query->where(function (Builder $q) use ($search, $searchUpper, $digits) {
                $q->where('id_cigam', 'like', "%{$search}%")
                    ...
                    ->orWhere('unidade_medicao', 'like', "%{$searchUpper}%")
                    ->orWhere('um_icms', 'like', "%{$searchUpper}%");
