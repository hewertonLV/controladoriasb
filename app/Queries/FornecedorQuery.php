...
class FornecedorQuery
{
    ...
    /**
     * @return array{search: string, per_page: int|string, id_estado: string|null, sort: string, direction: string}
     */
     ...
     * @param  array{search:string, per_page:int|string, id_estado:string|null, sort:string, direction:string}  $filtros
    ...
    public function aplicarFiltros(Builder $query, array $filtros): Builder
    {
        ...
        if ($sortKey === 'estado') {
            $query->leftJoin('estados', 'estados.id', '=', 'fornecedores.id_estado')
                ->select('fornecedores.*')
