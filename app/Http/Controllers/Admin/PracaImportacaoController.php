...
class PracaImportacaoController extends Controller
{
    ...
    {
        return view('admin.pracas.importar');
    }
    ...
    public function iniciar(Request $request): JsonResponse
    {
        ...
        $path = $file?->store('pracas/importacoes', 'local');
        ...
        ProcessarPreviewImportacaoPracasJob::dispatch($importacao->id);

        return response()->json([
            'uuid' => $importacao->uuid,
            'status' => $importacao->status,
            'urls' => [
                'status' => route('admin.pracas.importar.status', $importacao),
                'resultado' => route('admin.pracas.importar.resultado', $importacao),
                'confirmar' => route('admin.pracas.importar.confirmar', $importacao),
            ],
        ], 202);
    }

    public function status(Request $request, PracaImportacao $importacao): JsonResponse
    {
    ...
    public function resultado(Request $request, PracaImportacao $importacao): JsonResponse
    {
        $this->autorizarAcessoProcessamento($request, $importacao);
    ...
    public function confirmar(Request $request, PracaImportacao $importacao): JsonResponse
    {
        $this->autorizarAcessoProcessamento($request, $importacao);
        ...
        try {
            DB::transaction(function () use (
                ...
                foreach ($rowIdsNovas as $rowId) {
                    ...
                    if ($erroVal !== null) {
                        $erros[] = ['linha' => $dados['nome'] ?? "novas[{$rowId}]", 'erros' => [$erroVal]];
                        $ignoradas++;
                    ...
                    if (Praca::query()
                        ...
                        ->where('id_unidade_negocio', $dados['id_unidade_negocio'])
                        ->exists()) {
                        $ignoradas++;
                    ...
                    $this->auditoria->registrarCriacao(
                        $praca,
                        $user,
                        PracaHistorico::ORIGEM_IMPORTACAO_EXCEL,
                    );
                ...
                foreach ($rowIdsAtual as $rowId) {
                    ...
                    $pracaId = (int) ($item['praca_id'] ?? 0);
                    $praca = Praca::query()->find($pracaId);
                    if ($praca === null) {
                        $ignoradas++;
                    ...
                    $antesEsperado = $item['dados_atuais'] ?? [];

                    if (TextoCadastro::normalizarMaiusculas((string) ($antesEsperado['nome'] ?? '')) !== $praca->nome
                        || (int) ($antesEsperado['id_unidade_negocio'] ?? 0) !== (int) $praca->id_unidade_negocio) {
                        $erros[] = [
                            'linha' => $dados['nome'] ?? "atualizacoes[{$rowId}]",
                            'erros' => ['Inconsistência: praca_id não corresponde ao registro existente.'],
                        ];
                        $ignoradas++;
                    ...
                    $erroVal = $this->validarDados($dados);
                    if ($erroVal !== null) {
                        $erros[] = ['linha' => $dados['nome'] ?? "atualizacoes[{$rowId}]", 'erros' => [$erroVal]];
                        $ignoradas++;
            ...
            return response()->json([
                'message' => 'Erro ao gravar praças: '.$e->getMessage(),
            ], 500);
        }
     ...
     */
    private function statusPayload(PracaImportacao $importacao): array
    {
        return [
            'uuid' => $importacao->uuid,
            'status' => $importacao->status,
    ...
    private function removerArquivoTemporario(PracaImportacao $importacao): void
    {
        try {
