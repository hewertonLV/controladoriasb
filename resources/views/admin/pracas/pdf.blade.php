...
<head>
    <meta charset="UTF-8">
    <title>Relatório de Praças</title>
    <style>
...
<body>
    <h1>Relatório de Praças</h1>
    <p class="meta">Gerado em: {{ $geradoEm->format('d/m/Y H:i:s') }}</p>
    <p class="meta">Gerado por: {{ $geradoPor }}</p>
    <p class="meta">Total: {{ $pracas->count() }} praça(s) | Limite por PDF: {{ $limiteRegistros }}</p>
    <p class="meta">
        Filtros:
        @if (($filtros['search'] ?? '') !== '')
    ...
    <table>
        <thead>
            <tr>
                <th width="40%">Nome</th>
                <th width="20%">ID Unidade</th>
                <th width="40%">Unidade de negócio</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($pracas as $praca)
                <tr>
                    <td>{{ $praca->nome }}</td>
                    <td>{{ $praca->id_unidade_negocio }}</td>
                    <td>
                        @if ($praca->unidadeNegocio)
                            {{ $praca->unidadeNegocio->nome }} ({{ $praca->unidadeNegocio->id_cigam }})
                        @else
                            —
                        @endif
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="3" class="empty">Nenhuma praça corresponde aos filtros aplicados.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
