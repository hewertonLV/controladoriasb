...
@section('title', 'Importar Fretes')
@section('page-title', 'Importar Fretes')
...
@section('content')
        ...
        <div class="card-header d-flex flex-wrap align-items-center gap-2">
            <div class="me-auto">
                ...
                <p class="text-muted mb-0">
                    ...
                    <code>A</code> Nome · <code>B</code> Valor ·
                    <code>C</code> ID SBS veículo · <code>D</code> Descrição ·
                    <code>E</code> Situação · <code>F</code> Valor fruta/kg
                </p>
            </div>
            <a href="{{ route('admin.fretes.index') }}" class="btn btn-light">
                <i class="ri-arrow-left-line me-1"></i> Voltar para Fretes
            </a>
        </div>
        <div class="card-body">
        ...
        <div class="card mb-3" id="card-novas">
            <div class="card-header">
                <h5 class="header-title mb-0">
                    ...
                    Novas fretes <span class="badge bg-success-subtle text-success ms-1" id="count-novas">0</span>
                </h5>
            </div>
        ...
        <div class="card mb-3" id="card-atualizacoes">
            <div class="card-header">
                <h5 class="header-title mb-0">
                    ...
                    Fretes com alterações <span class="badge bg-warning-subtle text-warning ms-1" id="count-atualizacoes">0</span>
                </h5>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
    ...
    <div id="resumo-final" class="card d-none">
        ...
        <div class="card-body">
            ...
            <div class="d-flex gap-2 justify-content-end">
                ...
                <a href="{{ route('admin.fretes.index') }}" class="btn btn-primary">
                    <i class="ri-list-check-2 me-1"></i> Ir para Fretes
                </a>
            </div>
        </div>
    </div>
    ...
    (function () {
        ...
        const urlIniciar = @json(route('admin.fretes.importar.iniciar'));
        ...
        function fmtCampoValor(campo, v) {
            if (campo === 'valor' || campo === 'valor_fruta_kg') return fmtMoeda(v);
            if (campo === 'status_situacao') return v === 'ENCERRADA' ? 'Encerrada' : 'Aberta';
        ...
        function renderNovas(lista) {
            ...
            lista.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td><code>${escapeHtml(d.id_sbs || '—')}</code></td>
                    <td>${escapeHtml(d.status_situacao || '—')}</td>
                `;
        ...
        async function carregarResultadoEConfirmar() {
            try {
                ...
                if (preview.novas.length === 0 && preview.atualizacoes.length === 0) {
                    showAlerta('info', 'A planilha não trouxe novas fretes nem alterações para fretes existentes.');
                }
            } catch (err) {
        ...
        btnConfirmar.addEventListener('click', async () => {
            ...
            if (rowIdsNovas.length === 0 && rowIdsAtual.length === 0) {
                showAlerta('warning', 'Selecione ao menos um frete.');
                return;
            }
