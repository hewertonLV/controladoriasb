...
@section('title', 'Histórico — ' . $fruta->nome)
@section('page-title', 'Histórico da Fruta')
...
@section('content')
    <div class="card mb-3">
        <div class="card-header d-flex flex-wrap align-items-center gap-2">
            <div class="me-auto">
                <h4 class="header-title mb-1">{{ $fruta->nome }}</h4>
                <div class="text-muted small">
                    <code>{{ $fruta->id_cigam }}</code> ·
                    {{ $fruta->unidade_medicao }} ·
                    {{ number_format((float) $fruta->kg_por_unidade_medicao, 2, ',', '.') }} kg/un.
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($historicos as $h)
                            <tr>
                                ...
                                <td>
                                    @if ($h->origem === \App\Models\FrutaHistorico::ORIGEM_IMPORTACAO_EXCEL)
                                ...
                                <td>{{ $h->user?->name ?? '—' }}</td>
                                <td>
                                    @if ($h->acao === \App\Models\FrutaHistorico::ACAO_CRIACAO || $h->acao === \App\Models\FrutaHistorico::ACAO_IMPORTACAO_CRIACAO)
