...
@section('title', 'Histórico — ' . $veiculo->nome)
@section('page-title', 'Histórico do Veículo')
...
@section('content')
    <div class="card mb-3">
        <div class="card-header d-flex flex-wrap align-items-center gap-2">
            <div class="me-auto">
                <h4 class="header-title mb-1">{{ $veiculo->nome }}</h4>
                <div class="text-muted small">
                    <code>{{ $veiculo->id_sbs }}</code> ·
                    {{ $veiculo->id_unidade_negocio_formatado }}
                    @if ($veiculo->tipo)
                        · {{ $veiculo->tipo }}
                    @endif
                </div>
                    ...
                    <tbody>
                        @forelse ($historicos as $h)
                            <tr>
                                ...
                                <td>
                                    @if ($h->origem === \App\Models\VeículoHistorico::ORIGEM_IMPORTACAO_EXCEL)
                                ...
                                <td>{{ $h->user?->name ?? '—' }}</td>
                                <td>
                                    @if ($h->acao === \App\Models\VeículoHistorico::ACAO_CRIACAO || $h->acao === \App\Models\VeículoHistorico::ACAO_IMPORTACAO_CRIACAO)
