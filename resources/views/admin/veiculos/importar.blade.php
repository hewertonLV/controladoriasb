...
@section('page-title', 'Importar Veículos')
...
@section('content')
        ...
        <div class="card-header d-flex flex-wrap align-items-center gap-2">
            <div class="me-auto">
                ...
                <p class="text-muted mb-0">
                    ...
                    <code>A</code> ID SBS · <code>B</code> Nome ·
                    <code>C</code> Tipo · <code>D</code> ID Unidade de Negócio ·
                    <code>E</code> Status (ATIVO|INATIVO)
                </p>
            </div>
            <a href="{{ route('admin.veiculos.index') }}" class="btn btn-light">
                <i class="ri-arrow-left-line me-1"></i> Voltar para Veículos
            </a>
        </div>
        <div class="card-body">
        ...
        <div class="card mb-3" id="card-novas">
            <div class="card-header">
                <h5 class="header-title mb-0">
                    ...
                    Novas veiculos <span class="badge bg-success-subtle text-success ms-1" id="count-novas">0</span>
                </h5>
            </div>
        ...
        <div class="card mb-3" id="card-atualizacoes">
            <div class="card-header">
                <h5 class="header-title mb-0">
                    ...
                    Veículos com alterações <span class="badge bg-warning-subtle text-warning ms-1" id="count-atualizacoes">0</span>
                </h5>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
    ...
    <div id="resumo-final" class="card d-none">
        ...
        <div class="card-body">
            ...
            <div class="d-flex gap-2 justify-content-end">
                ...
                <a href="{{ route('admin.veiculos.index') }}" class="btn btn-primary">
                    <i class="ri-list-check-2 me-1"></i> Ir para Veículos
                </a>
            </div>
        </div>
    </div>
    ...
    (function () {
        ...
        const urlIniciar = @json(route('admin.veiculos.importar.iniciar'));
        ...
        function fmtCampoValor(campo, v) {
            if (campo === 'id_unidade_negocio') return fmtDoc(v);
        ...
        function renderNovas(lista) {
            ...
            lista.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${fmtDoc(d.id_unidade_negocio)}</td>
                `;
        ...
        function renderAtualizacoes(lista) {
            ...
            lista.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${item.linha}</td>
                    <td><code>${escapeHtml(item.id_sbs)}</code></td>
        ...
        async function carregarResultadoEConfirmar() {
            try {
                ...
                if (preview.novas.length === 0 && preview.atualizacoes.length === 0) {
                    showAlerta('info', 'A planilha não trouxe novas veiculos nem alterações para veiculos existentes.');
                }
            } catch (err) {
        ...
        btnConfirmar.addEventListener('click', async () => {
            ...
            if (rowIdsNovas.length === 0 && rowIdsAtual.length === 0) {
                showAlerta('warning', 'Selecione ao menos um veiculo.');
                return;
            }
