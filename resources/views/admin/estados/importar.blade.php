...
@section('page-title', 'Importar Estados')
...
@section('content')
        ...
        <div class="card-header d-flex flex-wrap align-items-center gap-2">
            <div class="me-auto">
                ...
                <p class="text-muted mb-0">
                    ...
                    <code>A</code> ID CIGAM · <code>B</code> Nome · <code>C</code> Abreviação · <code>D</code> Descrição (opcional)
                </p>
            </div>
            <a href="{{ route('admin.estados.index') }}" class="btn btn-light">
                <i class="ri-arrow-left-line me-1"></i> Voltar para Estados
            </a>
        </div>
        <div class="card-body">
        ...
        <div class="card mb-3" id="card-novas">
            <div class="card-header">
                <h5 class="header-title mb-0">
                    ...
                    Novos estados <span class="badge bg-success-subtle text-success ms-1" id="count-novas">0</span>
                </h5>
            </div>
        ...
        <div class="card mb-3" id="card-atualizacoes">
            <div class="card-header">
                <h5 class="header-title mb-0">
                    ...
                    Estados com alterações <span class="badge bg-warning-subtle text-warning ms-1" id="count-atualizacoes">0</span>
                </h5>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
    ...
    <div id="resumo-final" class="card d-none">
        ...
        <div class="card-body">
            ...
            <div class="d-flex gap-2 justify-content-end">
                ...
                <a href="{{ route('admin.estados.index') }}" class="btn btn-primary">
                    <i class="ri-list-check-2 me-1"></i> Ir para Estados
                </a>
            </div>
        </div>
    </div>
    ...
    (function () {
        ...
        const urlIniciar = @json(route('admin.estados.importar.iniciar'));
        ...
        function aplicarStatusNaUI(s) {
            ...
            if (s.status === 'AGUARDANDO') {
                progressoStatusTitulo.textContent = s.worker_status === 'INATIVO'
                    ? 'Worker de importação inativo'
        ...
        function renderNovas(lista) {
            ...
            lista.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td><span class="fw-semibold">${escapeHtml(d.nome)}</span></td>
                    <td><code>${escapeHtml(d.abreviacao)}</code></td>
        ...
        function renderAtualizacoes(lista) {
            ...
            lista.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td><span class="fw-semibold">${escapeHtml(item.dados_atuais.nome)}</span> <span class="text-muted small">(${escapeHtml(item.dados_atuais.abreviacao)})</span></td>
                    <td>${diffs}</td>
                `;
        ...
        function renderSemAlteracoes(lista) {
            ...
            lista.forEach(item => {
                ...
                tr.innerHTML = `
                    <td>${item.linha}</td>
                    <td><code>${escapeHtml(item.id_cigam)}</code></td>
                    <td><code>${escapeHtml(item.abreviacao)}</code></td>
                    <td><span class="fw-semibold">${escapeHtml(item.nome)}</span></td>
                `;
        ...
        async function carregarResultadoEConfirmar() {
            try {
                ...
                if (preview.novas.length === 0 && preview.atualizacoes.length === 0) {
                    showAlerta('info', 'A planilha não trouxe novos estados nem alterações para estados já cadastrados.');
                }
            } catch (err) {
        ...
        btnConfirmar.addEventListener('click', async () => {
            ...
            if (rowIdsNovas.length === 0 && rowIdsAtual.length === 0) {
                showAlerta('warning', 'Selecione ao menos um estado para importar.');
                return;
            }
