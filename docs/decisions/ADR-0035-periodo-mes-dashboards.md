...
**Data:** 2026-05-21
**Status:** Aceito
**Contexto:** Filtro de mês e botão Buscar nas dashboards principal e Olho de Deus

## Contexto

As dashboards usavam sempre o mês corrente implícito. A operação pediu seletor `type="month"` (Highdmin) com botão Buscar e, no Olho de Deus, dados do dia 01 até o dia atual no mês selecionado.
...
- Olho de Deus: `data_movimentacao` dentro do período; `carga_inicial=1` no Buscar varre o mês; polls seguintes só novidades (`since`) no mesmo período.
